...
This page is the compact capability map for the `ChatTodo` CLI. Use it to review which commands are first-class entries and which are still boundary or planned slots. After scaffolding, update it with the real command tree; do not present unimplemented commands as available operations.
...
```text
chattodo                  # ChatTodo command-line entry
...
```text
chattodo --help           # Verify the command is installed and inspect the current command tree
chattodo --version        # Verify the installed version
chattodo --tree           # Read back the CLI contract with parameter signatures
chattodo --tree-brief     # Read back command nodes and descriptions only
```
...
```text
chattodo <group>          # Command group named after real package capability
